...
const DeilveryRequestForm: React.FC = () => {
  const [selectedOption, setSelectedOption] =
    useState<string>("문 앞에 놔주세요");

  return (
    <div className="flex flex-col gap-1 w-full max-w-md">
      <label className="text-body-sb text-gray-600 mb-2 text-sm">
        배송 요청사항(선택)
      </label>
      <div className="flex flex-col">
        {deliveryOptions.map(option => (
          ...
          >
            <span className="text-body2">{option}</span>
        ...
        <input
          ...
          className="w-full border-b border-gray-300 py-3 text-body2 text-gray-400"
        />
      </div>
    </div>
  );
